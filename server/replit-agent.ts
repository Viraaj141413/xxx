...
export function generateReplitResponse(prompt: string) {
  ...
  const response = `🚀 **REPLIT AGENT MODE** - I'll create a ${projectDescription} from scratch!
...
### 🏗️ Project Structure (${files.length} files)
${files.map(file => `📄 \`${file}\``).join('\n')}

### 🛠️ Tech Stack (Replit Optimized)
...
## 💻 Generated Code (Agent Compatible)
